...
export default function ProfileScreen() {
  const { currentUser, setCurrentUser, setLinkedProfiles, discoverable, setDiscoverable, gridStatus, toast, setLoggedIn } = useApp();
  ...
  const handleChangeEmail = async () => {
    ...
    try {
      ...
      toast('Check your new email to confirm ✓');
      setShowChangeEmail(false);
      setNewEmail('');
    } catch (e) {
      // Demo mode — no real auth yet
      toast('Email updated ✓ (demo mode)');
      setShowChangeEmail(false);
      setNewEmail('');
    }
  };
  ...
  const handleChangePassword = async () => {
    ...
    try {
      ...
      toast('Password updated ✓ (demo mode)');
      setShowChangePassword(false);
      setNewPassword(''); setConfirmPassword('');
    }
  };
    ...
    <div>
      ...
      <div style={{ padding: '20px 16px 80px' }}>
        ...
        <div style={{ display: 'flex', gap: 16, alignItems: 'center', marginBottom: 16 }}>
          <div style={{ position: 'relative', flexShrink: 0 }}>
            ...
            <button onClick={() => setShowEdit(true)}
              style={{ position: 'absolute', bottom: -4, right: -4, width: 24, height: 24, borderRadius: '50%', background: C.sky, border: `2px solid ${C.bg}`, color: '#040f14', fontSize: 12, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
              ✏️
            </button>
          </div>
        ...
        <div style={{ display: 'flex', borderRadius: 14, overflow: 'hidden', border: `1px solid ${C.border}`, marginBottom: 16 }}>
          {[['Posts', 0], ['Followers', 0], ['Following', 5]].map(([label, val], i) => (
          ...
          <div style={{ background: C.card, borderRadius: '20px 20px 0 0', width: '100%', maxWidth: 480, padding: 20, maxHeight: '88vh', overflowY: 'auto' }} className="fadeUp">
            ...
            <div style={{ marginBottom: 14 }}>
              ...
              <div style={{ position: 'relative' }}>
                ...
                <button onClick={() => setShowCharPicker(showCharPicker === 'name' ? null : 'name')}
                  style={{ position: 'absolute', right: 10, top: '50%', transform: 'translateY(-50%)', fontSize: 16, color: showCharPicker === 'name' ? C.sky : C.muted }}>★</button>
              </div>
              {showCharPicker === 'name' && (
                <div style={{ marginTop: 6 }}>
                  <SLCharPicker onInsert={c => setEditDisplayName(n => n + c)} onClose={() => setShowCharPicker(null)} />
                </div>
              )}
            </div>
            ...
            <div style={{ marginBottom: 20 }}>
              ...
              <div style={{ position: 'relative' }}>
                ...
                <button onClick={() => setShowCharPicker(showCharPicker === 'bio' ? null : 'bio')}
                  style={{ position: 'absolute', right: 10, top: 10, fontSize: 16, color: showCharPicker === 'bio' ? C.sky : C.muted }}>★</button>
              </div>
              {showCharPicker === 'bio' && (
                <div style={{ marginTop: 6 }}>
                  <SLCharPicker onInsert={c => setEditBio(n => n + c)} onClose={() => setShowCharPicker(null)} />
                </div>
              )}
            </div>
        ...
        <div style={{ position: 'fixed', inset: 0, background: C.bg, zIndex: 700, display: 'flex', flexDirection: 'column', maxWidth: 480, margin: '0 auto' }} className="fadeUp">
          ...
          <div style={{ overflowY: 'auto', flex: 1, paddingBottom: 40 }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 14, padding: '13px 20px', borderBottom: `1px solid ${C.border}22` }}>
              ...
              <div style={{ flex: 1 }}>
                ...
                <div style={{ fontSize: 12, color: C.muted, marginTop: 2 }}>
                  {currentUser.showDisplayName !== false && currentUser.displayName
                    ? <><strong style={{ color: C.sky }}>{currentUser.displayName}</strong></>
                    : <><strong style={{ color: C.sky }}>@{currentUser.username}</strong></>}
                </div>
              </div>
            ...
            <div style={{ padding: '13px 20px', borderBottom: `1px solid ${C.border}22` }}>
              ...
              <div style={{ fontSize: 12, color: C.muted, marginTop: 2 }}>
                {currentUser.maturity === 'adult' ? '🔴 Adult' : currentUser.maturity === 'moderate' ? '🟡 Moderate' : '🟢 General'}
              </div>
            </div>
            ...
            <button onClick={() => setShowChangeEmail(!showChangeEmail)}
              ...
              <span style={{ color: C.muted }}>{showChangeEmail ? '↑' : '→'}</span>
            </button>
            {showChangeEmail && (
              <div style={{ padding: '12px 20px', borderBottom: `1px solid ${C.border}22`, background: `${C.sky}05` }}>
                <input value={newEmail} onChange={e => setNewEmail(e.target.value)} type="email"
                  placeholder="New email address" className="inp" style={{ marginBottom: 8 }} />
                <button onClick={handleChangeEmail}
                  style={{ width: '100%', padding: '10px', borderRadius: 12, background: `linear-gradient(135deg,${C.sky},${C.peach})`, color: '#060d14', fontWeight: 700, fontSize: 13 }}>
                  Update Email →
                </button>
              </div>
            )}
            ...
            <button onClick={() => setShowChangePassword(!showChangePassword)}
              ...
              <span style={{ color: C.muted }}>{showChangePassword ? '↑' : '→'}</span>
            </button>
            {showChangePassword && (
              <div style={{ padding: '12px 20px', borderBottom: `1px solid ${C.border}22`, background: `${C.sky}05` }}>
                <input value={newPassword} onChange={e => setNewPassword(e.target.value)} type="password"
                  placeholder="New password" className="inp" style={{ marginBottom: 8 }} />
                <input value={confirmPassword} onChange={e => setConfirmPassword(e.target.value)} type="password"
                  placeholder="Confirm new password" className="inp" style={{ marginBottom: 8 }} />
