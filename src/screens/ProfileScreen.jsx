  ...
  return (
    <div>
      ...
      <div style={{ padding: '20px 16px 80px' }}>
        ...
        {/* Stats */}
        <div style={{ display: 'flex', gap: 0, borderRadius: 14, overflow: 'hidden', border: `1px solid ${C.border}`, marginBottom: 16 }}>
          {[['Posts', 0], ['Followers', 0], ['Following', 5]].map(([label, val]) => (
            <div key={label} style={{ flex: 1, textAlign: 'center', padding: '12px 0', background: C.card2, borderRight: `1px solid ${C.border}` }}>
        ...
        <div style={{ marginBottom: 16 }}>
          ...
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: 7 }}>
            {INTEREST_GROUPS.map(g => {
              const on = currentUser.groups?.includes(g.id);
              return (
                <button key={g.id}
                  onClick={() => {
            ...
            <div style={{ marginBottom: 14 }}>
              ...
              <div style={{ position: 'relative' }}>
                <input value={editDisplayName} onChange={e => setEditDisplayName(e.target.value)}
                  className="inp" style={{ paddingRight: 40 }} />
            ...
            <div style={{ marginBottom: 20 }}>
              ...
              <div style={{ position: 'relative' }}>
                <textarea value={editBio} onChange={e => setEditBio(e.target.value)}
                  className="inp" style={{ height: 80, paddingRight: 40 }} />
              ...
              <div style={{ flex: 1 }}>
                ...
                <div style={{ fontSize: 12, color: C.muted, marginTop: 2 }}>
                  {currentUser.showDisplayName !== false && currentUser.displayName
                    ? <>Showing <strong style={{ color: C.sky }}>{currentUser.displayName}</strong></>
                    : <>Showing <strong style={{ color: C.sky }}>@{currentUser.username}</strong></>}
                </div>
              </div>
